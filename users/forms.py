...
class EditProfileForm(Form):
    ...
    def clean_email(self):
        check_user = User.objects.filter(email=self.cleaned_data.get("email")).first()
...
class ContactForm(Form):
    ...
    message = CharField(
        max_length=500,
        required=True,
        widget=Textarea(attrs={"class": "form-control", "placeholder": "Message"}),
    )
